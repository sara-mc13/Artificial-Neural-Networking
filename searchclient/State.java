    ...
    private State(State parent, Action[] jointAction)
    {
        ...
        for (int agent = 0; agent < numAgents; ++agent)
        {
            ...
            switch (action.type)
            {
                ...
                case Push:
                    // As for agent simply add future move of agent to current agent location
                    this.agentRows[agent] += action.agentRowDelta;
                    this.agentCols[agent] += action.agentColDelta;

                    // Find current location of box by taking current location of agent
                    currBoxRow = this.agentRows[agent];
                    currBoxCol = this.agentCols[agent];
                    // Get the boxes value by putting that location into the 2d array of boxes
                    box = this.boxes[currBoxRow][currBoxCol];
                    // Nullify the current boxes location and place it in the future location
                    // Future box location is current box location + its future move
                    this.boxes[currBoxRow][currBoxCol] = 0;
                    this.boxes[currBoxRow + action.boxRowDelta][currBoxCol + action.boxColDelta] = box;

                    break;

                    // Pull does the samething in reverse
                case Pull:       
                    // Finds current location of box 
                    // by taking agent location + the opposite of direction of where the box is going
                    currBoxRow = this.agentRows[agent] - action.boxRowDelta;
                    currBoxCol = this.agentCols[agent] - action.boxColDelta;
                    box = this.boxes[currBoxRow][currBoxCol];
                    // Nullifies current location o fbox and then moves it to future
                    this.boxes[currBoxRow][currBoxCol] = 0;
                    this.boxes[this.agentRows[agent]][this.agentCols[agent]] = box;

                    // Simply adds future move to the agent's location
                    this.agentRows[agent] += action.agentRowDelta;
                    this.agentCols[agent] += action.agentColDelta;
                    break;
            }
    ...
    private boolean isApplicable(int agent, Action action)
    {
        ...
        switch (action.type)
        {
            ...
            case Push:
                ...
                if (box == 0 || this.boxColors[box - 'A'] != agentColor){
                    return false;
                }
        ...
        for (int agent = 0; agent < numAgents; ++agent)
        {
            ...
            switch (action.type)
            {
                ...
                case Pull:
                    ...
                    destinationCols[agent] = agentCol + action.agentColDelta;
                    break;
                    // boxRow = agentRow + (-1)*action.agentRowDelta;
                    // boxCol = agentCol + (-1)*action.agentColDelta;
                    // boxRows[agent] = boxRow;
                    // boxCols[agent] = boxCol;
                    // destinationRows[agent] = boxRow + (-1)*action.boxRowDelta;
                    // destinationCols[agent] = boxCol + (-1)*action.boxColDelta;
                    // break;
           }
        }
